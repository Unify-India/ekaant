...
## 🟡 **5. Cloud Messaging Triggers – *For push notifications logic***

Not a separate function type, but often used inside functions.
...
* **sendNotification()** for:
  ...
  * Over-occupancy warnings
  * Payment pending alerts
  * Daily reminders

👉 Used *inside* other function types for push notifications.
...
| ---------------------- | ----------------------------------- | ----------------------------------- |
| **onCall()**           | App → backend with auth             | bookings, payments, check-in/out    |
| **onRequest()**        | Public endpoints + external systems | library application form            |
